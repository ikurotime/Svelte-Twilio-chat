<script>
	...
	onMount(async () => {
		...
		currentChatname =
			currentServer[0]?.channels[0]?.friendly_name || $activeConversation.channelState.friendlyName;
		selectedChat.set({
			friendlyName: currentChatname,
			id: currentServer[0]?.channels[0]?.id,
			description: currentServer[0]?.channels[0]?.description
		});
...
<div class="flex w-full h-full flex-col">
	...
	<div class="flex flex-row h-full w-full">
		{#if innerWidth < 640}
			<div
				class="fixed h-full w-full mt-auto "
				use:draggable={{ axis: 'x', bounds: { right: -300 } }}
			>
				<div
					class={`handle bg-gray-600 dark:bg-[#363535] h-full w-full flex flex-col overflow-y-hidden text-gray-400`}
				>
					{#if currentServer}
		...
		{:else}
			<div
				class={` relative handle bg-gray-400 dark:bg-[#363535] h-full w-full flex-1	 flex flex-col  overflow-y-hidden text-gray-400`}
			>
				{#if $isChatLoading}
		...
		<div
			class="min-w-[250px] h-full max-h-[92vh] pt-8 px-3 overflow-y-scroll bg-gray-500 dark:bg-neutral-800"
		>
			<span class="text-gray-300 text-lg">- Participants -</span>
			{#each participants as participant}
				<div class="flex p-3 gap-3 items-center">
					<img
						src={participant?.state?.identity === $userName ||
						participant?.state?.identity === $user?.username
							? $session?.user_metadata?.avatar_url || $user.avatar || $discordUser?.user?.avatar
							: `https://avatars.dicebear.com/api/open-peeps/${participant?.state?.identity}.svg`}
						class="w-10 h-10 rounded-full bg-white "
						alt={$session?.user_metadata?.full_name}
					/>
					<span class={$colorNames[participant?.state?.identity?.length]}
						>{participant?.state?.identity}</span
					>
				</div>
			{/each}
		</div>
