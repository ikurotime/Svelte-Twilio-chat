<script>
	...
	onMount(async () => {
		...
		const { data } = await supabase
			.from('servers')
			.select(
				`id,friendly_name, channels!channels_server_id_fkey(id, server_id,description,friendly_name)`
			)
...
<div class="grid place-content-center h-full">
	...
	{:else if isNewUser}
		<div
			class="grid place-content-center w-full sm:w-96 h-full sm:h-72 p-56 bg-neutral-600 rounded text-white shadow-lg gap-10 text-center"
		>
			<h1>How would you like to be known?</h1>
			<h2 class="text-3xl">
				{username}
			</h2>
