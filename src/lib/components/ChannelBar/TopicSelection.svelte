...
import { activeConversation, isChatLoading, lastAccessToken, selectedChat } from '$lib/stores/store';
	...
	async function changeChat() {
		...
		selectedChat.set(topic.friendlyName);
		console.log(topic);
...
<div on:click={changeChat} class={`dropdown-selection ${$selectedChat === topic.friendlyName ? 'bg-gray-400 dark:bg-neutral-600' : ''} hover:bg-gray-300 dark:hover:bg-neutral-500 py-2 px-3 mr-2 w-full dark:text-gray-500 dark:hover:text-white rounded `}>
	<FaHashtag />
	<h5 class={`dropdown-selection-text ${$selectedChat === topic.friendlyName ? 'text-green-400 dark:text-green-500' : ''}`}>{topic.friendlyName}</h5>
</div>
