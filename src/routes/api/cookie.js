...
export const post = async ({ request }) => {
  ...
  return {
    status: 200,
    headers: {
      'set-cookie': `session=${JSON.stringify(session)}; Path=/; HttpOnly; Secure; SameSite=Strict; Expires=${session.expires_at ? new Date(session.expires_at * 1000).toUTCString() : new Date(Date.now() + 1000 * 60 * 60).toUTCString()}`
    }
  }
}
